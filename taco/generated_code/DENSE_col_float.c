...
int compute(taco_tensor_t *out, taco_tensor_t *mtx_kddcupdense_0, taco_tensor_t *vec_rand) {
  int out1_dimension = (int)(out->dimensions[0]);
  float* restrict out_vals = (float*)(out->vals);
  float out_fill_value = *((float*)(out->fill_value));
  int mtx_kddcupdense_01_dimension = (int)(mtx_kddcupdense_0->dimensions[0]);
  int mtx_kddcupdense_02_dimension = (int)(mtx_kddcupdense_0->dimensions[1]);
  float* restrict mtx_kddcupdense_0_vals = (float*)(mtx_kddcupdense_0->vals);
  ...
  for (int32_t j_crd = 0; j_crd < vec_rand1_dimension; j_crd++) {
    for (int32_t i_crd = 0; i_crd < mtx_kddcupdense_02_dimension; i_crd++) {
      int32_t imtx_kddcupdense_0_pos = j_crd * mtx_kddcupdense_02_dimension + i_crd;
      out_vals[i_crd] = out_vals[i_crd] + mtx_kddcupdense_0_vals[imtx_kddcupdense_0_pos] * vec_rand_vals[j_crd];
    }
  }
