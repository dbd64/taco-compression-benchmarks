...
int compute(taco_tensor_t *out, taco_tensor_t *mtx_kddcupsparse_0, taco_tensor_t *vec_rand) {
  int out1_dimension = (int)(out->dimensions[0]);
  float* restrict out_vals = (float*)(out->vals);
  float out_fill_value = *((float*)(out->fill_value));
  int mtx_kddcupsparse_01_dimension = (int)(mtx_kddcupsparse_0->dimensions[0]);
  int* restrict mtx_kddcupsparse_02_pos = (int*)(mtx_kddcupsparse_0->indices[1][0]);
  int* restrict mtx_kddcupsparse_02_crd = (int*)(mtx_kddcupsparse_0->indices[1][1]);
  float* restrict mtx_kddcupsparse_0_vals = (float*)(mtx_kddcupsparse_0->vals);
  ...
  for (int32_t j_crd = 0; j_crd < vec_rand1_dimension; j_crd++) {
    for (int32_t imtx_kddcupsparse_0_pos = mtx_kddcupsparse_02_pos[j_crd]; imtx_kddcupsparse_0_pos < mtx_kddcupsparse_02_pos[(j_crd + 1)]; imtx_kddcupsparse_0_pos++) {
      int32_t i_crd = mtx_kddcupsparse_02_crd[imtx_kddcupsparse_0_pos];
      out_vals[i_crd] = out_vals[i_crd] + mtx_kddcupsparse_0_vals[imtx_kddcupsparse_0_pos] * vec_rand_vals[j_crd];
    }
  }
